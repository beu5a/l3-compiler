...
abstract class CPSOptimizer[T <: SymbolicNames](protected val treeModule: T)(
    using
    TypeTest[treeModule.Atom, treeModule.Literal],
  ...
  private def shrinkLetP(tree: LetP, s: State): Tree = {
    tree match {
      ...
      case LetP(name, prim, Seq(x: Literal, y), body)
          if leftNeutral.contains((x, prim)) =>
        shrink(body, s.withASubst(name, y))
      case LetP(name, prim, Seq(x, y: Literal), body)
          if rightNeutral.contains((prim, y)) =>
        shrink(body, s.withASubst(name, x))
      case LetP(name, prim, Seq(x: Literal, y), body)
          if leftAbsorbing.contains((x, prim)) =>
        shrink(body, s.withASubst(name, x))
      case LetP(name, prim, Seq(x, y: Literal), body)
          if rightAbsorbing.contains((prim, y)) =>
        shrink(body, s.withASubst(name, y))

      // identity
      case LetP(name, L3Primitive.Id, Seq(a), body) =>
        shrink(body, s.withASubst(name, a))

      // CSE
      case LetP(name, prim, args, body)
          if s.eInvEnv.contains((prim, args map s.aSubst)) && !impure(
            prim
          ) && !unstable(prim) =>
      ...
      case LetP(name, prim, l @ Seq(x: Literal, y: Literal), body)
          if vEvaluator.isDefinedAt(prim, l) =>
  ...
  private def shrink(tree: Tree, s: State): Tree = {
    tree match {
      case LetF(funs, body) => {
        val cntNotDead = funs.filter(f => !s.dead(f.name))
        val onceCnt = cntNotDead.filter(f => s.appliedOnce(f.name))
        val notOnce = cntNotDead.filter(f => !s.appliedOnce(f.name))
        val updatedState = s.withFuns(onceCnt)

        if notOnce.isEmpty then shrink(body, updatedState)
        else {
          val shrunkFuns = notOnce.map(f =>
            Fun(f.name, f.retC, f.args, shrink(f.body, updatedState))
          )
          LetF(shrunkFuns, shrink(body, updatedState))
        }

      }

      case LetC(cnts, body) => {
        val cntNotDead = cnts.filter(c => !s.dead(c.name))
        val onceCnt = cntNotDead.filter(c => s.appliedOnce(c.name))
        val notOnce = cntNotDead.filter(c => !s.appliedOnce(c.name))
        val updatedState = s.withCnts(onceCnt)

        if notOnce.isEmpty then shrink(body, updatedState)
        else {
          val shrunkCnts =
            notOnce.map(c => Cnt(c.name, c.args, shrink(c.body, updatedState)))
          LetC(shrunkCnts, shrink(body, updatedState))
        }
      }

      case If(cond, args, thenC, elseC) if cEvaluator.isDefinedAt(cond, args) =>
        val res = cEvaluator(cond, args)
        shrink(AppC(if (res) then thenC else elseC, Seq()), s)
      case If(cond, Seq(v1, v2), thenC, elseC) if v1 == v2 => {
        if (sameArgReduceC(cond)) shrink(AppC(s.cSubst(thenC), Seq()), s)
        else shrink(AppC(s.cSubst(elseC), Seq()), s)
      }

      case If(cond, args, thenC, elseC) =>
        If(cond, args map s.aSubst, s.cSubst(thenC), s.cSubst(elseC))

      case t: LetP => shrinkLetP(t, s)

      case AppC(cnt, args) if s.hasCnt(cnt, args) => {
        val nCnt = s.cEnv(cnt)
        shrink(nCnt.body, s.withASubst(nCnt.args, args))
      }

      case AppC(cnt, args) => AppC(s.cSubst(cnt), args map s.aSubst)

      case AppF(fun: Name, retC, args) if s.hasFun(fun, args) => {
        val nFun = s.fEnv(fun)
        shrink(
          nFun.body,
          s.withASubst(nFun.args, args).withCSubst(nFun.retC, retC)
        )
      }
    ...
    val trees = LazyList.iterate((0, tree), fibonacci.length) { (i, tree) =>
      ...
      def inlineT(tree: Tree)(using s: State): Tree = {
        tree match {
          ...
          case LetF(funs, body) => {
            val fFuns = funs filter { f => size(f.body) <= funLimit }
            val updatedState = s.withFuns(fFuns)
            val newFuns = funs map { f => f.copy(body = inlineT(f.body)) }
            LetF(newFuns, inlineT(body)(using updatedState))
          }
          case LetC(cnts, body) =>
            val fcnts = cnts.filter { c => size(c.body) <= cntLimit }
            val updatedState = s.withCnts(fcnts)
            val newCnts = cnts map { c => c.copy(body = inlineT(c.body)) }

            LetC(newCnts, inlineT(body)(using updatedState))
  ...
  protected val vEvaluator: PartialFunction[
    (ValuePrimitive, Seq[Atom]),
    Literal
  ]
  protected val cEvaluator: PartialFunction[(TestPrimitive, Seq[Atom]), Boolean]
}

object HighCPSOptimizer
    extends CPSOptimizer(HighCPSTreeModule)
  ...
  protected val vEvaluator
      : PartialFunction[(ValuePrimitive, Seq[Atom]), Literal] =
  ...
  protected val cEvaluator
      : PartialFunction[(TestPrimitive, Seq[Atom]), Boolean] =
    case (IntLt, Seq(IntLit(x), IntLit(y))) => x < y
    case (IntLe, Seq(IntLit(x), IntLit(y))) => x <= y
    case (Eq, Seq(IntLit(x), IntLit(y)))    => x == y
}

object FlatCPSOptimizer
    extends CPSOptimizer(FlatCPSTreeModule)
